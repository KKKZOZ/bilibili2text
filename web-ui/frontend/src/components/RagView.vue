...
const loadIndexStatus = async () => {
  ...
  try {
    ...
    indexStatusError.value = err instanceof Error ? err.message : '获取状态失败';
  } finally {
...
const runIndexAll = async (force) => {
  ...
  try {
    ...
    const pendingText = Number.isFinite(pending) ? `，剩余 ${pending} 条未索引` : '';
...
const onAnswerClick = (e) => {
  ...
  const el = document.getElementById(target)
    || (Number.isFinite(targetNumber) ? document.getElementById(`source-${targetNumber - 1}`) : null);
  ...
  <div class="rag-root">
    ...
    <article class="panel search-panel">
      <header class="search-header">
        ...
        <h1>跨视频内容检索</h1>
        <p>基于历史转录内容，用自然语言提问，AI 从所有视频中检索相关片段并生成回答。</p>
      </header>
            ...
            <transition name="slide-down">
              <div v-if="showAuthorFilter" class="author-list">
                ...
                >
                  ...
                  <span class="author-count">{{ item.indexed_run_count }} 个视频</span>
                </label>
                <p v-if="authorList.length === 0" class="author-empty">暂无已索引的 UP主</p>
                <button
    ...
    <div v-if="!hasQueried" class="empty-state">
      ...
      <p class="empty-sub">AI 会从你的历史转录视频中找到相关内容片段，并给出综合回答。</p>
    </div>
        ...
        <div class="answer-header-row">
          ...
          <div v-if="answerDownloadId" class="answer-dl-actions">
            <button
              class="answer-dl-btn"
              :disabled="false"
              @click="download(`/api/download/${answerDownloadId}`, answerFilename)"
            >
              <FileText :size="13" />
              <span>MD</span>
            </button>
            <button
              ...
              @click="convertAndDownload(answerDownloadId, answerFilename, 'txt')"
            >
              <LoaderCircle v-if="isConverting(answerDownloadId, 'txt')" :size="13" class="spin" />
              <Type v-else :size="13" />
              <span>TXT</span>
            </button>
            <button
              ...
              @click="convertAndDownload(answerDownloadId, answerFilename, 'pdf')"
            >
              <LoaderCircle v-if="isConverting(answerDownloadId, 'pdf')" :size="13" class="spin" />
          ...
          >
            <div class="source-card-top">
              ...
              <div class="source-meta">
                <span class="source-title">{{ src.title || src.bvid || '未知视频' }}</span>
                <span v-if="src.bvid" class="source-bvid">
                  {{ src.bvid }}
                  <ExternalLink :size="11" />
                </span>
              </div>
              <div class="score-pill" :style="{ '--score-color': scoreColor(src.score) }">
            ...
            <div class="score-bar-track">
              <div
                ...
                :style="{ width: scorePercent(src.score) + '%', background: scoreColor(src.score) }"
              />
            </div>
            <p class="source-excerpt">{{ src.text.slice(0, 220) }}{{ src.text.length > 220 ? '…' : '' }}</p>
          </a>
        </div>
      </section>
    </transition>
    ...
    <section class="index-section">
      <button class="index-toggle" @click="toggleIndexPanel">
        <Database :size="14" />
        <span>索引管理</span>
        <ChevronDown :size="14" class="toggle-chevron" :class="{ open: showIndexPanel }" />
      </button>
        ...
        <div v-if="showIndexPanel" class="index-body">
          ...
          <div v-else-if="indexStatus" class="stats-grid">
            <div class="stat-card">
              ...
              <span class="stat-value">{{ indexStatus.total_indexed_runs }}</span>
              <span class="stat-label">已索引视频</span>
            </div>
            <div class="stat-card">
              <AlertCircle :size="18" class="stat-icon" />
              <span class="stat-value">{{ indexStatus.pending_index_runs }}</span>
          ...
          <div v-if="indexStatus?.indexed_items?.length" class="indexed-files">
            <button class="indexed-files-toggle" @click="showIndexedFiles = !showIndexedFiles">
              <span>当前已索引文件</span>
              <span class="indexed-files-toggle-meta">
                {{ indexStatus.indexed_items.length }} 条
                <ChevronDown :size="14" class="toggle-chevron" :class="{ open: showIndexedFiles }" />
              </span>
            </button>
            <div v-if="showIndexedFiles" class="indexed-file-list">
              ...
              >
                <div class="indexed-file-main">
                  <span class="indexed-file-title">{{ item.title || item.bvid || item.run_id }}</span>
                  <span v-if="item.author" class="indexed-file-author">{{ item.author }}</span>
                </div>
                <div class="indexed-file-meta">
                  <span
                    class="indexed-file-kind"
                    :class="{
                      'indexed-file-kind-summary': item.source_kind === 'summary',
                      'indexed-file-kind-markdown': item.source_kind === 'markdown',
                    }"
                  >{{ indexedFileKindLabel(item.source_kind) }}</span>
                  <span class="indexed-file-name">{{ item.source_filename }}</span>
                  <span class="indexed-file-chunks">{{ item.chunk_count }} chunks</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="index-actions">
            ...
            >
              <LoaderCircle v-if="isIndexing && !indexingForce" :size="14" class="spin" />
              增量索引
            </button>
            <button
          ...
          <p class="index-hint">「增量」跳过已索引的视频；「重建全部」清空后重新索引所有历史记录。</p>
        </div>
      </transition>
    </section>
  </div>
